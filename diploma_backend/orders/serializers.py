...
from .models import DeliverySettings, Order, OrderProduct
    ...
    def create(self, validated_data):
        ...
        for item in products_data:
            ...
            OrderProduct.objects.create(
                order=order, product=product, count=count, price=price
            )
...
class OrderDetailSerializer(serializers.ModelSerializer):
    ...
    def get_paymentError(self, obj: Order):
        ...
        """
        last_payment = (
            obj.payments.filter(status_payment="failed").order_by("-created_at").first()
        )
...
class ConfirmOrderSerializer(serializers.ModelSerializer):
    ...
    totalCost = serializers.DecimalField(
        max_digits=10, decimal_places=2, source="total_cost"
    )
    ...
    def update(self, instance, validated_data):
        ...
        order_total_cost = sum(
            item.price * item.count for item in instance.products.all()
        )
        ...
        if instance.delivery_type == "express":
            ...
            if order_total_cost > settings_delivery.free_threshold:
                ...
                instance.total_cost = (
                    order_total_cost + settings_delivery.standard_price
                )
