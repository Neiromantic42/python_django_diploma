from django.contrib.auth.models import \
    User  # Импорт встроенной модели пользователя Django
...
def profile_avatar_upload_to_path(instance: "ImagesProfile", filename: str) -> str:
    ...
    return "profiles/profile_{pk}/avatar/{filename}".format(
        pk=instance.profile.user.pk, filename=filename
    )
...
class Profile(models.Model):
    ...
    user = models.OneToOneField(  # Связь с моделью юзер один к одному
        User, on_delete=models.CASCADE, related_name="profile"  # Имя для обратной связи
    )
    middle_name = models.CharField(
        max_length=20, blank=True, null=True, verbose_name="отчество"
    )
    phone = PhoneNumberField(
        null=True, blank=False, unique=True, region="RU", verbose_name="Телефон"
    )
...
class ImagesProfile(models.Model):
    ...
    alt = models.CharField(
        max_length=255, blank=True, verbose_name="Текстовое описание изображения"
    )
